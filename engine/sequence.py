...
class Sequence:
    ...
    def __getstate__(self):
        return (self.num_tokens, self.num_prompt_tokens, self.num_computed_tokens, self.block_table,
                self.token_ids if self.num_completion_tokens == 0 else self.last_token)

    def __setstate__(self, state):
        self.num_tokens, self.num_prompt_tokens, self.num_computed_tokens, self.block_table = state[:-1]
